...
RSpec.describe Product, type: :model do
  describe 'validate a new product' do
    ...
    it "should require a title no shorter than 2 symbols" do
      expect(FactoryGirl.build(:product, :title => "a" * 1)).to be_invalid
    end

    it "should require a title no longer than 60 symbols" do
    ...
    it "should require a short description" do
      expect(FactoryGirl.build(:product, :short_description => "")).to be_invalid
    end

    it "should require a short description no shorter than 2 symbols" do
      expect(FactoryGirl.build(:product, :short_description => "a" * 1)).to be_invalid
    end

    it "should require a short description no longer than 100 symbols" do
      expect(FactoryGirl.build(:product, :short_description => "a" * 101)).to be_invalid
    end

    it "should require a long description" do
      expect(FactoryGirl.build(:product, :long_description => "")).to be_invalid
    end

    it "should require a long description no shorter than 2 symbols" do
      expect(FactoryGirl.build(:product, :long_description => "a" * 1)).to be_invalid
    end

    it "should require a long description no longer than 500 symbols" do
